...
{
#ifdef __ANDROID__
  LOGD("returning NULL in pn_connector in driver.c");
#endif
  if (!driver) return NULL;
  ...
  struct addrinfo *addr;
#ifdef __ANDROID__
  LOGD("calling getaddrinfo in pn_connector in driver.c");
#endif
  int code = getaddrinfo(host, port, NULL, &addr);
  if (code) {
...
#ifdef __ANDROID__
  LOGD("calling pn_create_socket in pn_connector in driver.c");
#endif
...
#ifdef __ANDROID__
  LOGD("calling pn_configure_sock in pn_connector in driver.c");
#endif
  pn_configure_sock(sock);
...
#ifdef __ANDROID__
  LOGD("calling freeaddrinfo in pn_connector in driver.c");
#endif
  freeaddrinfo(addr);
